...
def _lsof_foreign_db_holder_pids(watched: set) -> Optional[List[int]]:
    ...
    Note ``lsof`` can exit 0 while warning that it "can't stat()" an unrelated
    stale network mount, producing a PARTIAL listing — which is exactly why an
    unparseable/empty result is not treated as "nobody holds it".
    """
    import subprocess
...
def _foreign_db_holder_pids(db_path: Path) -> Optional[List[int]]:
    ...
    macOS/BSD, which has no ``/proc``. An already-unlinked ``(deleted)``
    sidecar generation — the #90950 split-brain fingerprint — still counts as
    held on both paths. Returns ``None`` when the holder scan is UNAVAILABLE,
    which callers must treat as "ownership unknown", never as "nobody holds
    it".

    This function used to return ``None`` on every non-Linux platform, so each
    caller's ``if holders:`` guard was permanently falsy and every
    sidecar-unlink path failed OPEN on macOS — the 2026-09-21 split-brain in
    which ``_safe_restore_db`` unlinked sidecars the gateway still had open.
    """
...
def _safe_restore_db(src: Path, dst: Path) -> bool:
    ...
    try:
        ...
        try:
            ...
            # ``None`` means the holder scan itself was UNAVAILABLE (no lsof, lsof
            # failed, /proc unreadable) — NOT "nobody holds it". lsof on macOS also
            # exits 0 while warning that it "can't stat()" a stale network mount, so a
            # partial scan can silently omit the real holder. Treating that as "no
            # holders" is the exact macOS fail-open that produced the #90950
            # split-brain, so refuse instead of unlinking sidecars we cannot prove
            # are unheld. (Kept OUT of the auto-restore path in
            # ``update_cmd_maint._restore_state_db_from_snapshot``, which must still
            # proceed on ``None``: gateways are already drained there, and refusing on
            # unknown would disable auto-restore entirely.)
            if holders is None:
                logger.error(
                    ...
                    "unlinks live -wal/-shm and must fail closed.",
                    dst,
                )
                return False
            if holders:
...
def _import_db_member(
    ...
    if not target.exists():
        ...
        # Deliberately NOT refusing on ``None`` here, unlike the unlink+move restore
        # path. This branch only fires when *target* is ABSENT, so there is no live
        # file being replaced and nothing to lose; ``hermes import`` is the
        # disaster-recovery path, and a missing ``lsof`` must not be able to block
        # recreating a database the user has already lost. ``None`` now only means
        # "no scan at all" (Linux uses /proc; lsof ships with macOS/BSD).
        if holders:
            raise OSError(
