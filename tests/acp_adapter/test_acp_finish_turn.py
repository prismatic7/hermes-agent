...
@pytest.mark.asyncio
async def test_finish_turn_transformed_after_stream_reuses_streamed_message_id():
    """The transform delta must land in the SAME bubble as the streamed text.

    ACP clients group chunks by ``messageId`` and APPEND within one id, so the
    post-stream delta has to reuse the streamed message's id — allocating a new
    one would open a second bubble and the delta would read as a stray fragment.
    """
    acp_agent, state, _fake, conn = make_agent_and_state()
    state.is_running = True
    ...
    streamed_id = state.message_ids.current()
    # End-of-stream flush: Hermes core sends the ``None`` sentinel, which closes
    # the active id. This is the real state at _finish_turn time — without it
    # ``current()`` returns the still-open streamed id and the assertion below
    # could pass even when the delta wrongly opens a new bubble.
    ...
    sent = [u for _sid, u in conn.updates if getattr(u, "session_update", None) == "agent_message_chunk"]
    assert [u.content.text for u in sent] == ["\n\n[plugin appended this]"]
    assert sent[0].message_id == streamed_id, "delta must reuse the streamed bubble's message id"
    # The turn is sealed: the next chunk must allocate a fresh id.
    assert state.message_ids.current() != streamed_id


@pytest.mark.asyncio
async def test_finish_turn_unstreamed_opens_its_own_message_id():
    """An unstreamed final response opens its own bubble under the allocator."""
    acp_agent, state, _fake, conn = make_agent_and_state()
    state.is_running = True
    from acp_adapter.events import AssistantMessageIdAllocator
    ...
    sent = [u for _sid, u in conn.updates if getattr(u, "session_update", None) == "agent_message_chunk"]
    assert [u.content.text for u in sent] == ["full answer"]
    assert sent[0].message_id is not None


@pytest.mark.asyncio
async def test_finish_turn_transformed_empty_delta_sends_no_chunk():
    """A pure no-op transform (empty delta) must not emit an empty chunk."""
    acp_agent, state, _fake, conn = make_agent_and_state()
    state.is_running = True
    from acp_adapter.events import AssistantMessageIdAllocator

    state.message_ids = AssistantMessageIdAllocator()
    state.message_ids.current()

    result = {
        "final_response": "same text",
        "messages": [{"role": "assistant", "content": "same text"}],
        "response_transformed": True,
        "pre_transform_response": "same text",
    }
    await acp_agent._finish_turn(state, state.session_id, conn, result, None, streamed_message=True)

    assert _text_updates(conn) == []
